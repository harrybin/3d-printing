...
const VIEW_STORAGE_KEY = 'stl-canvas-view-defaults'
const defaultModelFile = new URLSearchParams(window.location.search).get('model') || ''
...
function readViewDefaults() {
  try {
    ...
    return { ...fallbackView, ...sanitizeView(JSON.parse(raw)) }
  } catch {
    return { ...fallbackView }
  }
}
...
function loadModel(file, preserveView) {
  ...
  return fetch(`${modelUrl(file)}?t=${Date.now()}`, { cache: 'no-store' })
    .then((r) => {
      if (!r.ok) throw new Error(`Model file not found: models/${file}`);
    ...
    .then((buffer) => {
      ...
      const s = parsed || {};
      const b = s.bounds;
      const trianglesData = Array.isArray(s.triangles) ? s.triangles : [];
      ...
      applyModelRotation();
      currentFile = file;
      return readModelVersion(file).then((version) => {
