...
- **Moving internal divider ribs / segment separators:** treat each separator as a constrained parametric curve, never as a translated mesh fragment. Freeze the outer contour and the expected number of enclosed segments before editing. Move only the explicitly requested attachment points, then re-solve the Bézier control points between them:
  - extend an endpoint slightly *through* the outer frame only when that endpoint is intended to terminate at the frame; the frame boolean then trims the round stroke cap cleanly;
  - when an endpoint must attach to another internal divider, terminate on that divider with a small overlap, never beside it, so no visible round cap or unsupported stub remains;
  - to **stretch** a curve, move the attachment datums apart and preserve tangent directions; to **compress** it, move the control handles toward the chord between its fixed attachment datums while preserving the intended inflection of an S-curve;
  - constrain all intermediate curve samples to the intended chamber. They must not cross the outer contour, another separator, or re-enter the frame after leaving it. Only intended frame endpoints may cross the outer contour;
  - after every candidate, decompose the panel voids and assert the exact expected segment count. Reject candidates with merged chambers, extra sliver chambers, or a detached round endpoint even when the mesh itself is watertight.
- For a separator that repeatedly creates slivers or intersections, branch controlled candidates from the same frozen script parameters. Compare rendered candidates against the marked attachment landmarks; do not chain edits from a failed curve.
